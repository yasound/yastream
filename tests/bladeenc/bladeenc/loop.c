	    ...
	    III_psy_xmin *l3_xmin, int ch, int gr )
{
    ...
    if ( gr == 1 )
    {
        ...
        for ( gr2 = 0; gr2 < 2; gr2++ )
        {
            ...
            if ( ((int)xrmax[ch][gr2]) != 0.0 )							/* Looks strange! */
                condition++;
...
int INLINE bladTabValue( float in )
{
  ...
  if( retVal & (1<<15) ) {
    int *pTabEntry = (int *)bladeTablefloat;
    retVal &= 0x7fff;
    if( in2 < pTabEntry[retVal] ) {
        retVal--;
    }
  }
...
/*  for ( ; i < 576; i++ )
		ix[i] = 0; */
  if( i<576 )
     memset( &ix[i], 0, (576-i) * sizeof(ix[0]) );
...
/*____ xr_zero() ____________________________________________________________*/

int xr_zero( double	xr[576] )
{
	int		i;
	for( i = 0 ; i < 576 ; i++ )
		if( fabs(xr[i]) != 0.0 )
